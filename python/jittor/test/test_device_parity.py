...
Run::  python -m jittor.test.test_device_parity
"""
import unittest
...
# Compile serially for this verifier: with the parallel compiler, a compile error in
# one op's accelerator kernel (e.g. the known uint8 reduce-atomic gap) surfaces during a
# DIFFERENT test that happens to sync at that moment -- a cross-test "collateral" failure
# that mis-attributes the bug. Serial compile attributes each error to its own op.
try:
...
_KNOWN_DEVICE_ISSUES = {
    ...
    "median":  ("skip", "jittor median broken under torch-compat (misc.py:321 argsort unpack); see test_ops skip"),
