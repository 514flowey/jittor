...
// ***************************************************************
// jittor-core-gaps.md §3.6: independent, device-native stateful RNG
// generators, aligned with torch.Generator's contract --
...
// state -- not the pyjt-visible Generator wrapper -- is what a RandomOp/
// CurandRandomOp captures (by copying the shared_ptr) at construction time,
...
//
// This header must compile in a plain (non-CUDA-extension) translation
// unit -- it must not include <curand.h> or require -lcurand. The CUDA
// side of a generator's state (a curandGenerator_t) is therefore stored
// as an opaque void*, created/destroyed/reseeded only through function
// pointers that extern/cuda/curand registers at load time (the same
// "optional extension, looked up by string/registered hook, never a hard
// link-time dependency from core code" pattern random_op.cc already uses
// for the plain `curand_random` op).
#pragma once
#include "common.h"
#include <random>
...
// Registered by extern/cuda/curand/src/curand_wrapper.cc when that
// extension is loaded; remain null (and therefore unused) on a CPU-only
// build or before CUDA is initialized -- RandomGeneratorState only calls
// these when a CUDA device was actually requested.
