...
struct ForeignAllocator : Allocator {
    // -1 = CPU (preserves the original global singleton's exact behavior
    // below); >=0 = the physical GPU this foreign memory actually lives on
    // (set by DLPack import for a kDLCUDA source -- see src/pyjt/dlpack.cc).
    ...
    // is_cuda()/device_id() report the truth instead of always "CPU".
    int dev_id = -1;
    ...
    int device_id() const override { return dev_id; }
...
// (see src/pyjt/dlpack.cc's kDLCUDA import path).
